...
Common uses of type classes include:

### Serialization: `rediscala`

All the typeclasses in `rediscala` (for our definition of Type Class) are designed to serialize to and from byte strings. They don't have external instances. It would be interesting to see which projects that depend on `rediscala` create their own instances.

### BREADCRUMBS:

- `elastic4s` seems weird, it has HttpExecutable and Executable with a ton of instances. Look into that
- Monoids in general seem to appear in a couple of projects
- `algebird` implements instances of some typeclasses in `cats`
- This will be a lot more fun when we have a bigger sample, since we want to see what instances people create.
- Interesting cases:
 - Single instance of multiple parents

- `ensime` implements it's own "`S-primitive`s".
...
- Metascala `vm`:
    - Fully-fledged class, with vals, objects and defs inside.
    - Self-referencing implicit instance: `implicit val vm = this`
