...
draw_HOF = function (hof, where) {
  ggplot(hof, aes(name, occurrences)) +
    ...
    theme(legend.position="bottom", legend.direction="horizontal",
                     legend.title = element_blank()) +
    #labs(y = "# Conversions", x = "Project") +
    scale_x_discrete(label=function(x) substring(x, 0, 40)) +
...
types_HOF = function(df, title) {
  ...
  require(gridExtra)
  grid.arrange(
    draw_HOF(returns_hof, paste(title, "- Return Type")),
    draw_HOF(params_hof, paste(title, "- Parameter Type"))
  )
}
```
...
## Halls of fame for the 10 biggest projects
