...
object JsonQuery {
  ...
  def query[T: Decoder](input: String, m: Matcher[T], mx: Matcher[T]*): List[T] = {
    val combined = combineAnd(m +: mx)

    def pipeline[F[_]](implicit F: Sync[F]) =
      fs2.io.file.readAll[F](Paths.get(input), 4096)
  ...
  def queryAll[T: Decoder](inputs: Seq[String], m: Matcher[T], mx: Matcher[T]*): List[T] = {
    val combined = combineAnd(m +: mx)
